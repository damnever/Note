...
int Command_fetch(apr_pool_t *p, const char *url, int fetch_only)
{
    ...
    if (apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS) {
        ...
        check(!fetch_only, "No point to fetching a DEPENDS file.");
        ...
        if (info.scheme) {
            ...
            check(rc == 0, "Failed to curl src: %s", TAR_BZ2_SRC);
        }
        ...
        sentinel("Donot know how to handle %s", url);
    }
        ...
        const char *make_opts, const char *install_opts)
{
    ...
    check(access(BUILD_DIR, X_OK | R_OK | W_OK) == 0,
            "Build irectory does not exist: %s", BUILD_DIR);

    // autual do and install
        ...
        const char *make_opts, const char *install_opts)
{
    ...
    rc = Shell_exec(CLEANUP_SH, NULL);
    check(rc == 0, "Failed to clean up before build.");
